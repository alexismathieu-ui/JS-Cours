...
class Game {
  ...
  update(gameStateFromServer) {
    ...
    //Update players
    const serverPlayerId = new Set(Object.keys(gameStateFromServer.players));
    // Add or update players from server data
    for (const playerId of serverPlayerId) {
    ...
    for (const playerId of Object.keys(this.players)) {
      if (!serverPlayerId.has(playerId)) {
...
const modifiedBackendData = backendData;
console.log(
  "Le Joueur",
...
modifiedBackendData.players["garlic"] = {
  name: "Cristalline",
  skinPath: "./assets/21.png",
  position: [0.426812357, 0.96486521545],
  lvl: 1,
  hp: 100,
  maxHp: 100,
  hpRegenRate: 10,
...
console.log(
  "Le Joueur",
  modifiedBackendData.players["garlic"].name,
  "A rejoint la partie"
);

modifiedBackendData.players["28ead291-fcea-4b41-a596-d3c876c49a53"].hp = 80;
game.update(modifiedBackendData);
